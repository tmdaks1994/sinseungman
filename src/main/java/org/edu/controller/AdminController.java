...
public class AdminController {
	...
	@Inject
	IF_BoardService boardService; //게시판 인터페이스 주입받아 boardService오브젝트 변수를 생성.
	...
	public String board_list(PageVO pageVO, Model model) throws Exception {
		...
		pageVO.setTotalCount(countBoard);//x개 전체 회원의 수를 구한 변수 값 매개변수로 입력하는 순간 calcPage()메서드실행.
	...
	public String member_delete(RedirectAttributes rdat, @RequestParam("user_id") String user_id) throws Exception {
		...
		//Redirect로 페이지 이동시 전송값을 숨겨서 보내는 역할 클래스 RedirectAttributes 입니다.
	...
	public String member_list(@ModelAttribute("pageVO") PageVO pageVO, Model model) throws Exception {
		...
		pageVO.setTotalCount(countMember);//전체 회원의 수를 구한 변수 값 매개변수로 입력하는 순간 calcPage()메서드실행.
