              ...
              <!-- /.card-header -->
                ...
                <div class="card-body">
                  ...
                  <div class="form-group">
                    ...
                    <input value="${memberVO.user_name}" name="user_name" type="text" class="form-control" id="user_name" placeholder="이름을 입력해주세요."required>
					<!-- 필수입력 값은 html5에서 지원하는 유효성 검사중 required 속성을 사용해서 빈(null)체크(유효성검사)를 한다. -->
                  </div>
                  <div class="form-group">
			...
			<!-- 버튼영역 시작 -->
              <div class="card-body">
				<a href="/admin/member/member_list?page=${pageVO.page}" class="btn btn-primary float-right mr-1">LIST ALL</a>
              	<a href="/admin/member/member_view?page=${pageVO.page}&user_id=${memberVO.user_id}" class="btn btn-info float-right mr-1">Back</a>
              	<button type="submit" class="btn btn-danger float-right mr-1">Modified</button>             
              	<!-- a태그는 링크이동은 되지만, post값을 전송하지는 못한다. 그래서 button 태그를 사용 -->
              </div>
              </div>
            <!-- 버튼영역 끝 -->
